...
use xml_dom::{get_implementation, Name, RefNode};
...
fn test_create_attribute() {
    ...
    assert!(attribute.owner_document().is_none());
...
fn test_create_cdata_section() {
    ...
    assert!(cdata_section.owner_document().is_none());
...
fn test_create_comment() {
    ...
    assert!(comment.owner_document().is_none());
...
fn test_create_processing_instruction() {
    ...
    assert!(processing_instruction.owner_document().is_none());
    let expected_name = Name::from_str("test").unwrap();
...
fn test_create_text_node() {
    ...
    assert!(text.owner_document().is_none());
